...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final int categoryCrossAxisCount = screenWidth > 800 ? 5 : 4;
    final double categoryAspectRatio = screenWidth > 800 ? 1.0 : 0.8;
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            ...
            // All 9 products in a single grid
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16),
              child: GridView.count(
                ...
                children: [
                  _buildCategory("চাল", Icons.rice_bowl_outlined, imagePath: 'image/rice.png'),
                  _buildCategory("ডাল", Icons.eco_outlined, imagePath: 'image/dal.png'),
                  _buildCategory("সয়াবিন তেল", Icons.opacity, imagePath: 'image/oil.png'),
                  _buildCategory("চিনি", Icons.grain, imagePath: 'image/sugar.png'),
                  _buildCategory("আটা", Icons.bakery_dining_outlined, imagePath: 'image/ata.png'),
                  _buildCategory("পেঁয়াজ", Icons.grass, imagePath: 'image/piyaj.png'),
                  _buildCategory("আলু", Icons.fiber_manual_record, imagePath: 'image/alu.png'),
                  _buildCategory("ডিম", Icons.egg_outlined, imagePath: 'image/dim.png'),
                  _buildCategory("মুরগি", Icons.set_meal, imagePath: 'image/murgi.png'),
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _buildSummaryCard("মোট অভিযোগ", "৫", Icons.description_outlined, const Color(0xFFF1F7FF), const Color(0xFF2196F3)),
              const SizedBox(width: 10),
              _buildSummaryCard("তদন্তাধীন", "১", Icons.search_rounded, const Color(0xFFF8F7FF), const Color(0xFF673AB7)),
              const SizedBox(width: 10),
              _buildSummaryCard("নিষ্পত্তি হয়েছে", "২", Icons.check_circle_outline_rounded, const Color(0xFFF2FAF5), const Color(0xFF4CAF50)),
            ],
          ),
        ],
      ),
  ...
  Widget _buildInfoCards() {
    return Padding(
      ...
      child: Row(
        children: [
          _buildServiceCard("অন্যান্য অভিযোগ", "নকল, মেয়াদোত্তীর্ণ,\nওজন কম ইত্যাদি", Icons.verified_user, const Color(0xFFF5F3FF), const Color(0xFF6366F1)),
          const SizedBox(width: 8),
          _buildServiceCard("ভোক্তা শিক্ষা", "আপনার অধিকার\nসম্পর্কে জানুন", Icons.menu_book, const Color(0xFFF0FDF4), const Color(0xFF22C55E)),
          const SizedBox(width: 8),
          _buildServiceCard("নোটিশ ও ঘোষণা", "সরকারি নোটিশ ও\nসর্বশেষ আপডেট", Icons.campaign, const Color(0xFFFFFBEB), const Color(0xFFF59E0B)),
        ],
      ),
    );
  }

  Widget _buildServiceCard(String title, String sub, IconData icon, Color bgColor, Color iconColor) {
    return Expanded(
      child: Container(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 10),
            Text(title, textAlign: TextAlign.center, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.black87)),
            const SizedBox(height: 4),
            Text(sub, textAlign: TextAlign.center, style: TextStyle(fontSize: 9, color: Colors.grey.shade600, height: 1.2)),
          ],
        ),
      ),
    );
  ...
  Widget _buildCategory(String label, IconData icon, {String? imagePath, VoidCallback? onTap}) {
    return InkWell(
      onTap: onTap ?? () {},
      borderRadius: BorderRadius.circular(12),
      child: Column(
            ...
            child: imagePath != null
                ? ClipRRect(
                    ...
                    child: Image.asset(
                      ...
                      height: 44,
                      fit: BoxFit.cover,
                      errorBuilder: (context, error, stackTrace) => Icon(icon, color: Colors.orange.shade700, size: 30),
                    ),
                  )
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(desc, style: TextStyle(fontSize: 10, color: Colors.grey.shade600), maxLines: 1, overflow: TextOverflow.ellipsis),
            const SizedBox(height: 12),
            Row(
              children: [
                Text("আরও দেখুন", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: accentColor)),
...
class _ScrollingNoticeState extends State<ScrollingNotice> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Container(
      height: 38,
      width: double.infinity,
      decoration: BoxDecoration(color: Colors.green.shade50, border: Border(top: BorderSide(color: Colors.green.shade100, width: 0.5), bottom: BorderSide(color: Colors.green.shade100, width: 0.5))),
      child: Row(
        children: [
          Container(
            ...
            child: const Text("বিজ্ঞপ্তি", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
          ),
          Expanded(
            child: ClipRect(
              child: AnimatedBuilder(
                ...
                child: Text(widget.text, style: const TextStyle(color: Color(0xFF1B5E20), fontWeight: FontWeight.w500, fontSize: 14), softWrap: false),
              ),
            ),
          ),
        ],
...
class _AnimatedComplaintButtonState extends State<AnimatedComplaintButton> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _dotColorAnimation = ColorTween(begin: Colors.red, end: Colors.green).animate(_controller);
    _bgColorAnimation = ColorTween(begin: Colors.white, end: Colors.red.shade50.withValues(alpha: 0.8)).animate(_controller);
    _glowAnimation = Tween<double>(begin: 2.0, end: 10.0).animate(CurvedAnimation(parent: _controller, curve: Curves.easeInOut));
  }
        ...
        builder: (context, child) {
          return Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(20),
              border: Border.all(color: _dotColorAnimation.value!.withValues(alpha: 0.3), width: 1.5),
              boxShadow: [BoxShadow(color: _dotColorAnimation.value!.withValues(alpha: 0.2), blurRadius: _glowAnimation.value, spreadRadius: _glowAnimation.value / 4)],
            ),
            child: Center(
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Container(width: 10, height: 10, decoration: BoxDecoration(color: _dotColorAnimation.value, shape: BoxShape.circle, boxShadow: [BoxShadow(color: _dotColorAnimation.value!.withValues(alpha: 0.6), blurRadius: 6, spreadRadius: 2)])),
                  const SizedBox(width: 8),
                  const Flexible(child: Text("অভিযোগ করুন", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Color(0xFF1B5E20)), overflow: TextOverflow.ellipsis)),
                ],
              ),
            ),
          );
